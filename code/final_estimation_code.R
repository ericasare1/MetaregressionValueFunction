...
lnacres_lnwtp <- df %>%
	...
	geom_point() +
	geom_text(aes(label=us),hjust=1, vjust=-1, size=1.3) + theme_bw() +
...
lm <- lm(lnwtp ~ q01 + lnyear + us + 
		  	local + 
		  	prov + reg + cult + lninc +
		  	forest + #q0 + q1 +
		  	volunt + lumpsum + ce + nrev + lnq_change, data  = df1)

car::vif(lm) #income will be dropped cos of its high VIF
#b) ...Outliers...
boxplot(df$lnwtp) #no problem with outliers
